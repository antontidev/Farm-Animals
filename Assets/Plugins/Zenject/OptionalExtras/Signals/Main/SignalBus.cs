...
namespace Zenject
{
    public class SignalBus : ILateDisposable
    {
        ...
        public void SubscribeLazy<TSignal>(Action<TSignal> callback) {
            if (HasSubscribe(callback)) return;
            
            Subscribe(callback);
        }

        public bool HasSubscribe<TSignal>(Action<TSignal> callback) {
            var signalId = new BindingId(typeof(TSignal), null);
            var id = new SignalSubscriptionId(signalId, callback);
            return _subscriptionMap.ContainsKey(id);
        }
        
        public bool HasSubscribe<TSignal>(Action callback) {
            var signalId = new BindingId(typeof(TSignal), null);
            var id = new SignalSubscriptionId(signalId, callback);
            return _subscriptionMap.ContainsKey(id);
        }
